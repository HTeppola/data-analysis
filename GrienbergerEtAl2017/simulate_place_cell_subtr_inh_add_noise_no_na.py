...
rec_filename = 'output'+datetime.datetime.today().strftime('%m%d%Y%H%M')+'-pid'+str(os.getpid())+'-seed'+\
               ...
               '-noise'+str(noise_rate)+'_no_na_'+str(trial_seed)
...
def get_dynamic_theta_phase_force(phase_ranges, peak_loc, input_field_duration, stim_t, dt):
    ...
    for i in range(len(phase_ranges)-1):
        ...
        if (abs(del_t) > 0.) and (abs(del_phase) > 0.):
...
def run_trial(simiter):
    ...
    for group in stim_exc_syns:
        peak_noise_rate = 2. * noise_rate / (2. - excitatory_theta_modulation_depth[group])
        for i, syn in enumerate(stim_exc_syns[group]):
            ...
            gauss_force = (excitatory_peak_rate[group] - peak_noise_rate) * \
                          np.exp(-((stim_t - peak_locs[group][i]) / gauss_sigma)**2.) + peak_noise_rate
...
excitatory_precession_range['CA3'] = [(-input_field_duration*0.5, 0.), (-input_field_duration*0.5, 180.),
                                      (-input_field_duration*0.35, 180.), (input_field_duration*0.35, -180.),
                                      (input_field_duration*0.5, -180.), (input_field_duration*0.5, 0.)]
...
sim.parameters['noise_rate'] = noise_rate
