...
for tried in range(8):
    ...
    for ele in sorted(index, reverse=True):
        ...
        #    spk=spk[:,tmp]
        TS = np.max(spk[1])
        a = int(np.round(np.min(STspk[1]) * 1. / 1000, 0))
        b = int(np.round(np.max(STspk[1]) * 1. / 1000, -1))
            ...
            for i in np.arange(a + j, b, 4):
                if np.max(y[np.where((x > i) & (x < i + offset))[0]]) > 100 and np.max(
                    ...
                    if isRandomST(STspkX, STspkY, i):
                        ...
                        cluster.append(pivot[j])
                        #    for i in np.arange(a+1,b,4):
                        #        if np.max(y[np.where((x>i)&(x<i+offset))[0]])>100:
                        #            plt.plot([i,i],[0,4096],'r',lw=2)
                        #            cluster.append(1)
                        #    for i in np.arange(a+2,b,4):
                        #        if np.max(y[np.where((x>i)&(x<i+offset))[0]])>100:
                        #            plt.plot([i,i],[0,4096],'r',lw=2)
                        #            cluster.append(2)
                        #    for i in np.arange(a+3,b,4):
                        #        if np.max(y[np.where((x>i)&(x<i+offset))[0]])>100:
                        #            plt.plot([i,i],[0,4096],'y',lw=2)
                        #            cluster.append(3)
...
for i in xrange(len(testM.T)):
    for j in xrange(len(testM.T)):
        testM[i, j] = ttest_rel(test[:, i], test[:, j])[1]
